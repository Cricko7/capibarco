...
func NewPublisher(brokers []string, clientID string, retryCount int, backoff time.Duration, failThreshold uint32, logger *slog.Logger) *Publisher {
	return &Publisher{
		writer:     &kafka.Writer{Addr: kafka.TCP(brokers...), RequiredAcks: kafka.RequireOne, AllowAutoTopicCreation: true, Balancer: &kafka.Hash{}, Transport: &kafka.Transport{ClientID: clientID}},
		logger:     logger,
		retryCount: retryCount,
		backoff:    backoff,
		breaker:    gobreaker.NewCircuitBreaker(gobreaker.Settings{Name: "kafka-publisher", MaxRequests: 3, Interval: 30 * time.Second, Timeout: 15 * time.Second, ReadyToTrip: func(c gobreaker.Counts) bool { return c.ConsecutiveFailures >= failThreshold }}),
	}
}
