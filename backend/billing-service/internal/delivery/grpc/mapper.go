...
func moneyFromProto(m *commonv1.MoneyAmount) (domain.Money, error) {
	if m == nil {
		return domain.Money{}, fmt.Errorf("%w: amount is required", domain.ErrValidation)
	}
	return domain.NewMoney(m.GetCurrencyCode(), m.GetUnits(), m.GetNanos())
}
...
func donationTargetFromProto(t billingv1.DonationTargetType) domain.DonationTargetType {
	switch t {
	case billingv1.DonationTargetType_DONATION_TARGET_TYPE_SHELTER:
		return domain.DonationTargetShelter
	case billingv1.DonationTargetType_DONATION_TARGET_TYPE_ANIMAL:
		return domain.DonationTargetAnimal
	default:
		return domain.DonationTargetUnspecified
	}
}
...
func entitlementTypeFromProto(t billingv1.EntitlementType) domain.EntitlementType {
	switch t {
	case billingv1.EntitlementType_ENTITLEMENT_TYPE_ADVANCED_FILTERS:
		return domain.EntitlementAdvancedFilters
	case billingv1.EntitlementType_ENTITLEMENT_TYPE_EXTENDED_ANIMAL_STATS:
		return domain.EntitlementExtendedAnimalStats
	case billingv1.EntitlementType_ENTITLEMENT_TYPE_ANIMAL_BOOST:
		return domain.EntitlementAnimalBoost
	default:
		return ""
	}
}
...
func ledgerToProto(e domain.LedgerEntry) *billingv1.LedgerEntry {
