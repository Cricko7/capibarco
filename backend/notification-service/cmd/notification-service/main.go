...
func run() error {
	...
	service := app.NewService(repo, publisher, cfg.Service.Name, cfg.Kafka.TopicPrefix, time.Now, logger)
	...
	notificationv1.RegisterNotificationServiceServer(grpcServer, deliverygrpc.NewServer(service))

	httpServer := deliveryhttp.NewServer(cfg.HTTP.Addr, cfg.HTTP.ReadTimeout, cfg.HTTP.WriteTimeout, cfg.HTTP.IdleTimeout, cfg.HTTP.CORSOrigins, repo, m, logger)
